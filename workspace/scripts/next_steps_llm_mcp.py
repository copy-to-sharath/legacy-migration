...
def update_reqnroll_project(vector_cites: List[str]) -> None:
    TEST_PROJECT_DIR.mkdir(parents=True, exist_ok=True)
    csproj = TEST_PROJECT_DIR / "ReqnrollTests.csproj"
    if csproj.exists():
        text = csproj.read_text(encoding="ascii")
        if "Reqnroll.Tools.MsBuild.Generation" not in text:
            text = text.replace(
                "</ItemGroup>",
                "    <PackageReference Include=\"Reqnroll.Tools.MsBuild.Generation\" Version=\"2.1.0\" />\n  </ItemGroup>",
                1,
            )
        if "ReqnrollFeatureFiles" not in text:
            text = text.replace(
                "</Project>",
    ...
    smoke_test = TEST_PROJECT_DIR / "SmokeTests.cs"
    smoke_test.write_text(
        "\n".join(
            [
                ...
                "{",
                "    [Test]",
                "    public void ReqnrollProjectLoads()",
                "    {",
                "        Assert.Pass();",
                "    }",
                "}",
...
def main() -> int:
    ...
    update_reqnroll_project(vector_cites)
